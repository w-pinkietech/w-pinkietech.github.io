...
const CLIEmulator: React.FC<CLIEmulatorProps> = ({ initialOutput = [] }) => {
  ...
  const [lines, setLines] = useState<OutputLine[]>(
    initialOutput.map(line => ({ type: 'output', content: line }))
  );
  const [history, setHistory] = useState<string[]>([]);
  const [historyIndex, setHistoryIndex] = useState<number>(-1);
  const [currentLang, setCurrentLang] = useState<'ja' | 'en'>('ja');
  const [isWaitingForInput, setIsWaitingForInput] = useState(false);
  const [inputPrompt, setInputPrompt] = useState('guest@pinkietech:~$');
  const [commandContext, setCommandContext] = useState<string>('');
  ...
  const addLine = (type: 'output' | 'input', content: string, prompt?: string) => {
    setLines(prev => [...prev, { type, content, prompt }]);
  };
  ...
  const handleCommand = (command: string) => {
    ...
    if (isWaitingForInput) {
      // Remove the input prompt line and add the actual input
      setLines(prev => prev.slice(0, -1));
      addLine('input', command, inputPrompt);
      if (commandContext === 'sudo') {
        if (command) {
          addLine('output', currentLang === 'ja' ? '権限がありません。ナイストライ！ 😏' : 'Permission denied. Nice try! 😏');
        }
        setIsWaitingForInput(false);
        setInputPrompt('guest@pinkietech:~$');
        setCommandContext('');
        // Add new prompt line
        addLine('input', '', 'guest@pinkietech:~$');
      } else if (commandContext === 'mail_subject') {
        newOutput.push(inputPrompt + ' ' + command);
        if (command) {
          newOutput.push(currentLang === 'ja' ? '本文: ' : 'Message: ');
          setInputPrompt(currentLang === 'ja' ? '本文' : 'Message');
        ...
        newOutput.push(inputPrompt + ' ' + command);
        newOutput.push('');
        newOutput.push(currentLang === 'ja' ? '✓ メールを送信しました（デモ）' : '✓ Email sent (demo)');
        setIsWaitingForInput(false);
        setInputPrompt('guest@pinkietech:~$');
        setCommandContext('');
      } else if (commandContext === 'confirm_exit') {
        newOutput.push(inputPrompt + ' ' + command);
        if (command.toLowerCase() === 'y' || command.toLowerCase() === 'yes') {
          newOutput.push(currentLang === 'ja' ? 'さようなら！' : 'Goodbye!');
          setTimeout(() => {
    ...
    if (lowerCommand === 'help') {
      ...
      newOutput.push('');
      newOutput.push(currentLang === 'ja' ? 'メールクライアント v1.0' : 'Mail Client v1.0');
      newOutput.push(currentLang === 'ja' ? '宛先: info@pinkietech.jp' : 'To: info@pinkietech.jp');
      newOutput.push('');
      newOutput.push(currentLang === 'ja' ? '件名: ' : 'Subject: ');
      setIsWaitingForInput(true);
      setInputPrompt(currentLang === 'ja' ? '件名' : 'Subject');
      setCommandContext('mail_subject');
    } else if (lowerCommand === 'sudo' || lowerCommand.startsWith('sudo ')) {
      newOutput.push('[sudo] ' + (currentLang === 'ja' ? 'guestのパスワード: ' : 'password for guest: '));
      setIsWaitingForInput(true);
      setInputPrompt('[sudo] password: ');
      setCommandContext('sudo');
      ...
      newOutput.push('');
      newOutput.push(currentLang === 'ja' ? '本当に終了しますか？ (y/n): ' : 'Are you sure you want to exit? (y/n): ');
      setIsWaitingForInput(true);
      setInputPrompt('(y/n)');
      setCommandContext('confirm_exit');
    } else {
  ...
  const availableCommands = [
    'help', 'clear', 'home', 'about', 'services', 'works', 'projects',
    'news', 'contact', 'team', 'faq', 'legal', 'neofetch', 'info',
    'whoami', 'pwd', 'ls', 'date', 'echo', 'lang', 'language', 'cat',
    'mail', 'sudo', 'exit', 'quit', 'logout', 'hack', 'vim', 'vi', 'emacs', 'nano'
  ];
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      ...
      e.preventDefault();
      if (!isWaitingForInput && input.trim()) {
        ...
        if (matches.length === 1) {
          ...
          newOutput.push(matches.join('  '));
          setOutput(newOutput);
        }
      }
    } else if (e.key === 'ArrowUp') {
      e.preventDefault();
      if (!isWaitingForInput && history.length > 0 && historyIndex < history.length - 1) {
      ...
      e.preventDefault();
      if (!isWaitingForInput && historyIndex > 0) {
        const newIndex = historyIndex - 1;
        setInput(history[history.length - 1 - newIndex]);
        setHistoryIndex(newIndex);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center mt-4 pt-4 border-t border-pink-500/30">
        <span className="text-pink-300 mr-2 font-bold">{inputPrompt}</span>
        <input
          ref={inputRef}
          type={commandContext === 'sudo' ? 'password' : 'text'}
