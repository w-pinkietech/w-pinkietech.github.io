...
- Mobileメニューは、項目選択、メニュー外のクリック、`Escape`で閉じる。`Escape`で閉じた場合はトリガーへフォーカスを戻す。
...
- HeaderとHeroの相談CTAは、まずページ内の問い合わせ方法へ移動し、利用者が送信手段を選べるようにする。
- メール導線は`mailto:`だけに依存しない。宛先を画面へ明記し、「メールアドレスをコピー」を第一動線にする。
- `mailto:`は「メールアプリを開く」と明示した補助リンクにし、端末設定によって動作しない場合があることと、メールアプリでの送信操作が必要であることを伝える。

### Company profile

- 会社概要は独立したページとして設け、HeaderとFooterから到達できるようにする。
- 公開情報は`company-profile.md`を正本とし、会社名、代表取締役、設立、資本金、所在地、事業内容を保持する。
- 仮の電話番号、未提供のサービス、架空のニュース、確認できない実績や数値でページを埋めない。
- 全面改修でも、確認済みの会社情報を明示的な承認なく削除しない。
